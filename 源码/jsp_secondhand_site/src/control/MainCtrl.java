	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			System.out.println("dfs");
				...
				if (list.size() == 1) {
					...
					if (ulist.size() == 1&& password.equals(map.get("userpass").toString())) {
							...
							gor("/secondhand_site/admin/main.jsp", request, response);
			...
			String password = request.getParameter("upass"); 
				...
				if (list.size() == 1) {
					...
					if (ulist.size() == 1&& password.equals(map.get("userpass").toString())) {
							...
							gor("/secondhand_site/index.jsp", request, response);
		...
		if(ac.equals("uppass"))
		{
			...
			String copyuserpass = request.getParameter("copyuserpass");   
			 user = dao.getmap(Info.getUser(request).get("id").toString(), "users");
			if(!(((String)user.get("userpass")).equals(olduserpass)))
			{
				request.setAttribute("error", "");
				go("/admin/uppass.jsp", request, response);
			}else{
			String id = (String)user.get("id");
			String sql = "update users set userpass='"+userpass+"' where id="+id;
			dao.commOper(sql);
