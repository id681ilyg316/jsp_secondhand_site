...
<body>
<form  action="zxxscx.jsp"  name="f1" method="post">
          ...
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="97%" valign="bottom"><span class="STYLE1"> 二手交易管理</span></td>
              </tr>
            </table></td>
            <td> 
...
&nbsp;&nbsp;物品标题 &nbsp;:&nbsp; <input type=text class=''  size=15 name='title' />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;发布人 &nbsp;:&nbsp; <input type=text class=''  size=15 name='uname' />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;&nbsp;<input type=submit class='' value='查询信息' /> &nbsp;&nbsp;<input type=button   class=''  value='添加信息' onclick="add();" /> &nbsp;&nbsp;</td>
        </tr>
      
      
      <tr bgcolor="#F3F8F7">
        <td height="30" align="center"  style="background-color: #F3F8F7;color:#344B50;"  >物品标题</td>
        <td height="30" align="center"  style="background-color: #F3F8F7;color:#344B50;"  >物品类别</td>
...
<td height="30" align="center"  style="background-color: #F3F8F7;color:#344B50;"  >价格</td>
...
<td height="30" align="center"  style="background-color: #F3F8F7;color:#344B50;"  >操作</td>
      ...
      <% 
new CommDAO().delete(request,"secondhand"); 
if(request.getParameter("titleid1")!=null){ 
 new CommDAO().commOper("update secondhand set title ='' where id="+request.getParameter("titleid1"));  
} 
if(request.getParameter("titleid2")!=null){ 
 new CommDAO().commOper("update secondhand set title ='' where id="+request.getParameter("titleid2"));  
} 
String sql = "select * from secondhand where 1=1 " ;
 if(!ptitle.equals("")){ 
 ...
 sql+= " and uname <'"+Info.getDay(enduname,1)+"' " ;
  ...
  sql +="  and (1!=1 " ; 
  sql +="  or  infotype like'%二手交易%' " ;
  sql +="  ) ";
  sql +=" order by id desc ";
String url = "zxxscx.jsp?1=1&title="+ptitle+"&uname="+puname+""; 
ArrayList<HashMap> list = PageManager.getPages(url,5, sql, request ); 
for(HashMap map:list){ %>
<tr>

        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("title")%></td>
        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("wptype")%></td>
        ...
        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("lxaddrs")%></td>
        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("lxqq")%></td>
        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("price")%></td>
        ...
        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><a href="javascript:update('<%=map.get("id")%>')">修改</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="zxxscx.jsp?scid=<%=map.get("id")%>">删除</a></td>
</tr>
<%}%>
...
<script language=javascript> 
function update(no){ 
pop('zxxsxg.jsp?id='+no,'信息修改',550,394) 
}
</script> 
<script language=javascript> 
function add(){ 
pop('zxxstj.jsp','信息添加',550,394) 
}
</script> 
<%@page import="util.Info"%> 
