    ...
    <TD background=secondhand_files/bj.jpg align=middle>
      <TABLE border=0 cellSpacing=0 cellPadding=0 width=800>
       ...
       <jsp:include page="top.jsp"></jsp:include>
          ...
          <TD vAlign=top>
            <TABLE border=0 cellSpacing=0 cellPadding=0 width="100%">
              ...
              <TR>
                ...
                </div><form  action="ywyw.jsp?f=f&tglparentid=<%=tglparentid%>"  onSubmit="return checkform()"  name="f1" ><div align="left"> 
                </div><TABLE 
        ...
        PageManager pageManager = PageManager.getPage("ywyw.jsp",5, request);
	    pageManager.doList("select * from secondhand where infotype='物品租借'  and uname like '%"+ukey+"%' and  title like'%"+key+"%'   order by id desc");
	    ...
	    for(HashMap m :nlist)
	    {
           ...
           align=left vAlign=center class=css><a 
             href="fxiang.jsp?x=x&id=<%=m.get("id") %>"><%=m.get("title") %>&nbsp;&nbsp;<span class="hu_txt"><%=m.get("savetime").toString() %></span></a>
             ...
             {
             if(new CommDAO().select("select * from scj where uname='"+Info.getUser(request).get("uname")+"' and sctype='物品租借' and wid ='"+m.get("id")+"'").size()==0){
             %>
            <a href="ywyw.jsp?f=f&scwid=<%=m.get("id")%>">[收藏]</a>&nbsp;&nbsp;
             <%
             }
             else{%>
             [已收藏]
             ...
             if(Info.getUser(request)!=null)
             {
             if(new CommDAO().select("select * from scj where uname='"+Info.getUser(request).get("uname")+"' and sctype='租借分享' and wid ='"+m.get("id")+"'").size()==0){
             %>
            <a href="ywyw.jsp?f=f&fxwid=<%=m.get("id")%>">[分享]</a>&nbsp;&nbsp;
             <%
             }
             else{%>
             [已分享]
...
<tr>
                          <td width="159" height="30" align="center">新旧度</td>
...
<%}%>
//-->
</script> <script language=javascript src='/secondhand_site/js/My97DatePicker/WdatePicker.js'></script>
...
function update(no){ 
popheight="300";
pop('admin/ywywxg.jsp?id='+no,'信息修改',550,391) 
}
</script> 
